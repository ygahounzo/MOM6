...
!> Sets the initial velocity components to uniform
   subroutine initialize_velocity_uniform(u, v, G, GV, US, param_file, just_read)
      ...
      call get_param(param_file, mdl, "INITIAL_V_CONST", initial_v_const, &
         ...
         default=0.0, units="m s-1", scale=US%m_s_to_L_T, do_not_log=just_read)
      call get_param(param_file, mdl, "ADVECTION_TEST_X_ORIGIN", x_origin, &
         "The x-coordinate of the center of the test-functions.", units=G%x_ax_unit_short, default=0.)
      call get_param(param_file, mdl, "ADVECTION_TEST_Y_ORIGIN", y_origin, &
      ...
      do k=1,nz ; do j=js,je ; do I=Isq,Ieq
               u(I,j,k) = initial_u_const * cos(G%geoLatT(I,j)*dpi/180.0)
            enddo ; enddo ; enddo
      do k=1,nz ; do J=Jsq,Jeq ; do i=is,ie
