...
!> The control structure for the gyre_tracer module
type, public :: gyre_tracer_CS ; private
...
end type gyre_tracer_CS

contains

!> Register tracer fields and subroutines to be used with MOM.
function register_gyre_tracer(G, GV, param_file, CS, tr_Reg, restart_CS)
  type(ocean_grid_type),       intent(in) :: G    !< The ocean's grid structure
  type(verticalGrid_type),     intent(in) :: GV   !< The ocean's vertical grid structure
  type(param_file_type),       intent(in) :: param_file !< A structure to parse for run-time parameters
  type(gyre_tracer_CS), pointer :: CS !< The control structure returned by a previous
                                                !! call to register_gyre_tracer.
...
# include "version_variable.h"
  character(len=40)  :: mdl = "gyre_tracer" ! This module's name.
  ...
  if (associated(CS)) then
    call MOM_error(FATAL, "register_gyre_tracer called with an "// &
  ...
  do m=1,NTR
    ...
    if (CS%coupled_tracers) &
      CS%ind_tr(m) = atmos_ocn_coupler_flux(trim(name)//'_flux', &
          flux_type=' ', implementation=' ', caller="register_gyre_tracer")
  enddo

  CS%tr_Reg => tr_Reg
  CS%restart_CSp => restart_CS
  register_gyre_tracer = .true.
end function register_gyre_tracer

!>   Initializes the NTR tracer fields in tr(:,:,:,:) and it sets up the tracer output.
subroutine initialize_gyre_tracer(restart, day, G, GV, h,diag, OBC, CS, &
                                            sponge_CSp)
...
subroutine gyre_tracer_column_physics(h_old, h_new,  ea,  eb, fluxes, dt, G, GV, US, CS, &
...
end subroutine gyre_tracer_column_physics
...
subroutine gyre_tracer_surface_state(sfc_state, h, G, GV, CS)
  type(ocean_grid_type),   intent(in)    :: G  !< The ocean's grid structure.
  type(verticalGrid_type), intent(in)    :: GV !< The ocean's vertical grid structure
...
end subroutine gyre_tracer_surface_state
...
function gyre_stock(h, stocks, G, GV, CS, names, units, stock_index)
  type(ocean_grid_type),              intent(in)    :: G      !< The ocean's grid structure
  type(verticalGrid_type),            intent(in)    :: GV     !< The ocean's vertical grid structure
  ...
  integer                                           :: gyre_stock !< the number of stocks calculated here.
  ...
  do m=1,CS%ntr
    call query_vardesc(CS%tr_desc(m), name=names(m), units=units(m), caller="gyre_stock")
    stocks(m) = global_mass_int_EFP(h, G, GV, CS%tr(:,:,:,m), on_PE_only=.true.)
  enddo
  gyre_stock = CS%ntr

end function gyre_stock

!> Deallocate memory associated with this module
subroutine gyre_tracer_end(CS)
  type(gyre_tracer_CS), pointer :: CS !< The control structure returned by a previous
                                              !! call to register_gyre_tracer.
  if (associated(CS)) then
...
end subroutine gyre_tracer_end

end module gyre_tracer
