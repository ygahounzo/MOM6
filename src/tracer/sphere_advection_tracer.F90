...
!>   Initializes the NTR tracer fields in tr(:,:,:,:) and it sets up the tracer output.
   subroutine initialize_sphere_advection_tracer(restart, day, G, GV, h,diag, OBC, CS, &
      sponge_CSp)
      ...
      do m=1,NTR
         call query_vardesc(CS%tr_desc(m), name=name, &
            caller="initialize_sphere_advection_tracer")
         if ((.not.restart) .or. (CS%tracers_may_reinit .and. .not. &
            ...
            do j=js,je ; do i=is,ie
                  ...
                  ! if (r >= RR) then
                  !   CS%tr(i,j,k,m) = 0.0
                  ! elseif ((abs(locx) <= 0.5*0.06) .and. (y <= (0.75+0.1))) then
                  !   CS%tr(i,j,k,m) = 0.0
                  ! else
                  !   CS%tr(i,j,k,m) = 1.0
                  ! endif
                  ...
                  ! ! if (locx<0.2 .and. abs(locy)<0.0) CS%tr(i,j,k,m) = 0.0
   ...
   end subroutine initialize_sphere_advection_tracer
...
!! from this package.  This is a simple example of a set of advected passive tracers.
   subroutine sphere_advection_tracer_column_physics(h_old, h_new,  ea,  eb, fluxes, dt, G, GV, US, &
...
!     h_new(k) = h_old(k) + ea(k) - eb(k-1) + eb(k) - ea(k+1)
   ...
   end subroutine sphere_advection_tracer_column_physics
...
!! This particular tracer package does not report anything back to the coupler.
   subroutine sphere_advection_tracer_surface_state(sfc_state, h, G, GV, CS)
      type(ocean_grid_type),   intent(in)    :: G  !< The ocean's grid structure.
      type(verticalGrid_type), intent(in)    :: GV !< The ocean's vertical grid structure
   ...
   end subroutine sphere_advection_tracer_surface_state
...
!!  If the stock_index is present, only the stock corresponding to that coded index is returned.
   function sphere_advection_stock(h, stocks, G, GV, CS, names, units, stock_index)
      type(ocean_grid_type),              intent(in)    :: G      !< The ocean's grid structure
      type(verticalGrid_type),            intent(in)    :: GV     !< The ocean's vertical grid structure
      ...
      integer                                           :: sphere_advection_stock !< the number of stocks calculated here.

      ! Local variables
      integer :: is, ie, js, je, nz, m
      is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec ; nz = GV%ke

      sphere_advection_stock = 0
      if (.not.associated(CS)) return
      if (CS%ntr < 1) return
      ...
      do m=1,CS%ntr
         call query_vardesc(CS%tr_desc(m), name=names(m), units=units(m), &
            caller="sphere_advection_stock")
         stocks(m) = global_mass_int_EFP(h, G, GV, CS%tr(:,:,:,m), on_PE_only=.true.)
      enddo
      sphere_advection_stock = CS%ntr

   end function sphere_advection_stock

!> Deallocate memory associated with this module
   subroutine sphere_advection_tracer_end(CS)
      type(sphere_advection_tracer_CS), pointer :: CS !< The control structure returned by a previous
      !! call to register_sphere_advection_tracer.
      if (associated(CS)) then
   ...
   end subroutine sphere_advection_tracer_end

end module sphere_advection_tracer
