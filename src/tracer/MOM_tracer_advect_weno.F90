...
pure subroutine weno5z_reconstruction_interface(wpl, qmm, qm, q0, qp, qpp, cfl, u)
  ...
  real :: qmp
  real :: Tm, Tp, sg
  real :: q0_min, q0_max, Dq4

  !if (((maxval(cfl) > 0.4) .or. (abs(maxval(cfl)-minval(cfl)) > 0.1))) then
  if (abs(maxval(cfl)-minval(cfl)) > 0.1) then
    if (u < 0.0) then
      Tm = qp ; Tp = qm
    else
      Tm = qm ; Tp = qp
...
!!
!!  This program contains the subroutines that advect tracers horizontally
!!  (i.e. along layers) using high-order WENO schemes (Balsara et al., 2016)
